...
function initAdmin() {
  initSidebarNav();
  initSidebarToggle();
  // Ensure all sections hidden, then show dashboard
  ...
  try {
    ...
    list.innerHTML = videos.map(v => {
      ...
      <div style="display:flex;align-items:center;gap:1rem;padding:1rem;background:var(--warm-white);border-radius:12px;margin-bottom:0.75rem">
        ${v.thumbnail
          ? `<img src="${v.thumbnail}" style="width:80px;height:50px;object-fit:cover;border-radius:8px;flex-shrink:0">`
          : `<div style="width:80px;height:50px;background:var(--charcoal);border-radius:8px;display:grid;place-items:center;color:#fff;font-size:1.5rem;flex-shrink:0">🎬</div>`
        }
        <div style="flex:1;min-width:0">
          <div style="font-weight:700">${v.title || 'Untitled'}</div>
          <div style="font-size:0.8rem;color:var(--muted);margin-bottom:0.35rem">${v.tag || ''} · ${v.description || ''}</div>
          ${safeUrl
            ? `<div style="font-size:0.72rem;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:260px" title="${safeUrl}">${safeUrl}</div>
               <button onclick="window.previewAdminVideo('${safeUrl}')" style="margin-top:0.35rem;font-size:0.75rem;color:var(--deep-rose);background:none;border:none;cursor:pointer;padding:0;font-weight:600">▶ Preview video</button>`
            : `<div style="font-size:0.72rem;color:#E74C3C;font-weight:600">⚠️ No URL stored — re-upload this video</div>`
          }
        </div>
...
window.previewAdminVideo = (url) => {
  if (!url) return;
  document.getElementById('admin-video-preview-overlay')?.remove();

  const isVimeo = /vimeo\.com/i.test(url);
  const isImage = /unsplash\.com|images\.unsplash|\/image\/upload\//i.test(url);
  const vimeoMatch = isVimeo ? url.match(/vimeo\.com\/(?:video\/)?(\d+)/) : null;
  const vimeoId = vimeoMatch ? vimeoMatch[1] : null;

  let playerHtml = '';
  if (isVimeo && vimeoId) {
    playerHtml = `<iframe src="https://player.vimeo.com/video/${vimeoId}?autoplay=1&color=E8779A&title=0&byline=0" style="width:100%;aspect-ratio:16/9;border-radius:12px;border:none;background:#000" allow="autoplay;fullscreen" allowfullscreen></iframe>`;
  } else if (isImage) {
    playerHtml = `<img src="${url.replace(/"/g,'&quot;')}" style="max-width:100%;max-height:75vh;border-radius:12px;object-fit:contain">`;
  } else {
    playerHtml = `<video controls autoplay playsinline style="width:100%;border-radius:12px;max-height:75vh;background:#000" src="${url.replace(/"/g,'&quot;')}">Your browser does not support HTML5 video.</video>`;
  }

  const overlay = document.createElement('div');
  overlay.id = 'admin-video-preview-overlay';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:9999;display:grid;place-items:center;padding:1rem;cursor:pointer';
  overlay.innerHTML = `
    <div style="position:relative;width:100%;max-width:800px;cursor:default" onclick="event.stopPropagation()">
      <button style="position:absolute;top:-2.5rem;right:0;background:none;border:none;color:#fff;font-size:1.5rem;cursor:pointer" id="admin-video-close-btn">✕</button>
      ${playerHtml}
    </div>`;
  ...
  overlay.querySelector('#admin-video-close-btn').onclick = close;
...
document.getElementById('video-form')?.addEventListener('submit', async e => {
  ...
  try {
    ...
    // Also check for a thumbnail URL text input (for Unsplash / Cloudinary image links)
    let thumbUrl = (document.getElementById('vf-thumb-url')?.value || '').trim();

    if (videoFile) {
      videoUrl = await uploadToCloudinary(videoFile);
    }
    if (!videoUrl) { showAdminToast('Provide a video file or URL', 'error'); btn.disabled = false; btn.textContent = 'Add Video'; return; }

    if (thumbFile) {
      thumbUrl = await uploadToCloudinary(thumbFile);
    }

    // Auto-generate Vimeo thumbnail if none provided
    if (!thumbUrl && /vimeo\.com/i.test(videoUrl)) {
      const m = videoUrl.match(/vimeo\.com\/(?:video\/)?(\d+)/);
      if (m) thumbUrl = `https://vumbnail.com/${m[1]}.jpg`;
    }

    // If the URL itself looks like an image (Unsplash, Cloudinary image), use it as thumb too
    if (!thumbUrl && (/unsplash\.com/i.test(videoUrl) || /\/image\/upload\//i.test(videoUrl))) {
      thumbUrl = videoUrl;
    }
...
function initSidebarToggle() {
  // The toggle button and overlay click are wired in the inline script
  // (before this module loads) so they work immediately on page paint.
  // Here we just ensure nav link taps close the drawer on mobile.
  document.querySelectorAll('.sidebar-nav li a[data-section]').forEach(link => {
...
window.viewOrder = async (id) => window.viewOrderDetails(id);
