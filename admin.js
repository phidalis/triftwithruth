...
window.previewAdminVideo = (url) => {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:9999;display:grid;place-items:center;padding:1rem';
  overlay.innerHTML = `
    <div style="position:relative;width:100%;max-width:800px">
      ...
      <video controls autoplay playsinline style="width:100%;border-radius:12px;max-height:75vh;background:#000" src="${url.replace(/"/g,'&quot;')}">
        Your browser does not support HTML5 video.
      </video>
    </div>`;
  document.body.appendChild(overlay);
  overlay.querySelector('#admin-video-close-btn').onclick = () => {
    overlay.querySelector('video')?.pause();
    overlay.remove();
  };
  overlay.addEventListener('click', e => { if (e.target === overlay) { overlay.querySelector('video')?.pause(); overlay.remove(); } });
};
