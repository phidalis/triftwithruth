...
function renderVideos(videos) {
  ...
  const makeCard = (v, large) => `
    <div class="video-card">
      <div class="video-card-inner" onclick="playVideo('${v.url || ''}', '${v.title || ''}')">
        ${v.thumbnail
...
window.playVideo = (url, title) => {
  if (!url) return;
  let overlay = document.getElementById('video-lightbox');
  if (!overlay) {
    overlay = document.createElement('div');
    overlay.id = 'video-lightbox';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:9500;display:grid;place-items:center;padding:1rem';
    overlay.innerHTML = `
      <div style="position:relative;width:100%;max-width:900px">
        <button onclick="document.getElementById('video-lightbox').remove()"
          style="position:absolute;top:-2.5rem;right:0;background:none;border:none;color:#fff;font-size:1.5rem;cursor:pointer">✕</button>
        <video id="lightbox-video" controls autoplay style="width:100%;border-radius:12px;max-height:80vh"></video>
      </div>`;
