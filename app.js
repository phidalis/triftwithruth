...
function renderVideos(videos) {
  ...
  const makeCard = (v) => {
    ...
    // Resolve the best thumbnail:
    // 1. Explicit thumbnail saved in Firestore
    // 2. Auto-generated Vimeo thumb
    // 3. If the URL itself is an image (Unsplash), use it directly
    // 4. Cloudinary image URL saved as thumbnail
    let thumbSrc = v.thumbnail || '';
    if (!thumbSrc && vimeoId) thumbSrc = vimeoThumb(vimeoId);
    if (!thumbSrc && type === 'image') thumbSrc = url;  // Unsplash URL used as thumb

    const thumbHtml = thumbSrc
      ? `<img class="video-thumb" src="${thumbSrc}" alt="${safeTitle}" loading="lazy"
             onerror="this.style.display='none';this.nextElementSibling.style.display='grid'">`
      : '';
    const fallbackHtml = `<div class="video-thumb" style="background:linear-gradient(135deg,var(--deep-rose),#8B3059);display:${thumbSrc ? 'none' : 'grid'};place-items:center;font-size:3rem;width:100%;height:100%">▶️</div>`;
  ...
  if (videos.length === 1) {
    grid.innerHTML = makeCard(main);
    return;
  }
  const sideCards = rest.slice(0, 2).map(v => makeCard(v)).join('');
  grid.innerHTML = `
    ${makeCard(main)}
...
// Video lightbox — handles Vimeo embeds, direct mp4/Cloudinary, and image-only cards
window.playVideo = (url, title, type, vimeoId) => {
  if (!url) { console.warn('playVideo: no URL provided'); return; }
  // Resolve type if not passed (e.g. called from older markup)
  ...
  if (type === 'vimeo' && vimeoId) {
    // Vimeo: use their official oEmbed iframe
    playerHtml = `
      <iframe
    ...
    // Unsplash or image URL — show as a large preview (no video to play)
    playerHtml = `
  ...
  const closeAll = () => {
    ...
    overlay.querySelector('iframe')?.setAttribute('src', '');   // stops Vimeo audio
    overlay.remove();
  };

  overlay.addEventListener('click', closeAll);   // backdrop click
