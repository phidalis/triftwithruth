...
function renderVideos(videos) {
  ...
  if (!videos || !videos.length) {
    grid.innerHTML = '<p style="color:var(--muted);padding:2rem">No videos yet. Add some in the admin panel!</p>';
    return;
  }
  const [main, ...rest] = videos;
  const makeCard = (v, large) => {
    const safeUrl   = (v.url   || '').replace(/&/g, '&amp;');
    const safeTitle = (v.title || '').replace(/'/g, '&#39;').replace(/"/g, '&quot;');
    return `
    <div class="video-card">
      <div class="video-card-inner" data-video-url="${safeUrl}" data-video-title="${safeTitle}" onclick="playVideo(this.dataset.videoUrl, this.dataset.videoTitle)">
        ${v.thumbnail
          ? `<img class="video-thumb" src="${v.thumbnail}" alt="${safeTitle}" loading="lazy">`
          : `<div class="video-thumb" style="background:var(--petal);display:grid;place-items:center;font-size:3rem">▶</div>`
        }
        <div class="video-overlay">
          <button class="play-btn" aria-label="Play video">▶</button>
          <div class="video-info">
            <span class="video-tag">${v.tag || 'Video'}</span>
  ...
  if (videos.length === 1) {
    grid.innerHTML = makeCard(main, true);
    return;
  }
  const sideCards = rest.slice(0, 2).map(v => makeCard(v, false)).join('');
  grid.innerHTML = `
    ${makeCard(main, true)}
    <div style="display:flex;flex-direction:column;gap:1.5rem">${sideCards}</div>
  `;
}
...
window.playVideo = (url, title) => {
  ...
  // Remove any stale lightbox so video always resets cleanly
  document.getElementById('video-lightbox')?.remove();

  const overlay = document.createElement('div');
  overlay.id = 'video-lightbox';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:9500;display:grid;place-items:center;padding:1rem';
  overlay.innerHTML = `
    <div style="position:relative;width:100%;max-width:900px">
      <button onclick="document.getElementById('video-lightbox').remove()"
        style="position:absolute;top:-2.5rem;right:0;background:none;border:none;color:#fff;font-size:1.5rem;cursor:pointer;line-height:1" aria-label="Close">✕</button>
      ${title ? `<p style="color:rgba(255,255,255,0.7);font-size:0.9rem;margin-bottom:0.5rem">${title}</p>` : ''}
